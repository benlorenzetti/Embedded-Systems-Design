...
int main (int argc, char* argv[])
{
	...
	while (iter - rtttl_buffer < rtttl_size) {
		...
		if ((*iter >= '4') && (*iter <= '7'))
			...
			octave = default_oct;
		note_buffer[note_buffer_size] = (15 & duration) + (dotted << 3)
		note_buffer[note_buffer_size] += (note << 4) + (sharp << 7) + (octave << 8);
		if (*iter == ',')
	...
	printf ("\nPacked Data Array: ([Beats per Minute][Total Notes][Byte 1][Byte 2]...[Byte N])\n";
